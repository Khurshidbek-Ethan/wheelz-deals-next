...
import Review from '../../libs/components/vehicle/Review';
...
const vehicleDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [destinationvehicle, setDestinationvehicle] = useState<vehicle[]>([]);
	...
	if (device === 'mobile') {
		return <div>vehicle DETAIL PAGE</div>;
	} else {
		return (
			<div id={'vehicle-detail-page'}>
				<div className={'container'}>
					<Stack className={'vehicle-detail-config'}>
						<Stack className={'vehicle-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{vehicle?.vehicleTitle}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{vehicle?.VehicleLocation}</Typography>
										<Stack className={'divider'}></Stack>
										<Stack className={'buy-rent-box'}>
											{vehicle?.vehicleBarter && (
												<>
							...
							<Stack className={'images'}>
								<Stack className={'main-image'}>
									<img
										src={slideImage ? `${REACT_API_URL}/${slideImage}` : '/img/vehicle/bigImage.png'}
										alt={'main-image'}
									/>
								</Stack>
								<Stack className={'sub-images'}>
									{vehicle?.vehicleImages.map((subImg: string) => {
										const imagePath: string = `${REACT_API_URL}/${subImg}`;
										return (
						...
						<Stack className={'vehicle-desc-config'}>
							<Stack className={'left-config'}>
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{vehicle?.vehicleBeds}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{vehicle?.vehicleRooms}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{moment(vehicle?.createdAt).format('YYYY')}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{vehicle?.vehicleSquare} m2</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{vehicle?.VehicleType}</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										<Typography className={'title'}>vehicle Description</Typography>
										<Typography className={'desc'}>{vehicle?.vehicleDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>vehicle Details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Price</Typography>
													<Typography className={'data'}>${formatterStr(vehicle?.vehiclePrice)}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>vehicle Size</Typography>
													<Typography className={'data'}>{vehicle?.vehicleSquare} m2</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Rooms</Typography>
													<Typography className={'data'}>{vehicle?.vehicleRooms}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Bedrooms</Typography>
													<Typography className={'data'}>{vehicle?.vehicleBeds}</Typography>
												</Box>
											</Stack>
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Year Built</Typography>
													<Typography className={'data'}>{moment(vehicle?.createdAt).format('YYYY')}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>vehicle Type</Typography>
													<Typography className={'data'}>{vehicle?.VehicleType}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>vehicle Options</Typography>
													<Typography className={'data'}>
														For {vehicle?.vehicleBarter && 'Barter'} {vehicle?.vehicleRent && 'Rent'}
													</Typography>
												</Box>
											</Stack>
										</Stack>
								...
								<Stack className={'floor-plans-config'}>
									...
									<Stack className={'image-box'}>
										<img src={'/img/vehicle/floorPlan.png'} alt={'image'} />
									</Stack>
								</Stack>
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{vehicleComments?.map((comment: Comment) => {
								...
								<Stack className={'info-box'}>
									...
									<Stack className={'image-info'}>
										<img
											...
											src={
												vehicle?.memberData?.memberImage
													? `${REACT_API_URL}/${vehicle?.memberData?.memberImage}`
													: '/img/profile/defaultUser.svg'
											}
										/>
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${vehicle?.memberData?._id}`}>
												<Typography className={'name'}>{vehicle?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{vehicle?.memberData?.memberPhone}</Typography>
											</Stack>
						...
						{destinationvehicle.length !== 0 && (
							<Stack className={'similar-Vehicles-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Destination vehicle</Typography>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationvehicle.map((vehicle: vehicle) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={vehicle.vehicleTitle}>
