...
const Addvehicle = ({ initialValues, ...props }: any) => {
	...
	/** APOLLO REQUESTS **/
	let getvehicleData: any, getvehicleLoading: any;

	/** LIFECYCLES **/
	useEffect(() => {
		setInsertvehicleData({
			...insertvehicleData,
			vehicleTitle: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleTitle : '',
			vehiclePrice: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehiclePrice : 0,
			VehicleType: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.VehicleType : '',
			VehicleLocation: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.VehicleLocation : '',
			vehicleAddress: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleAddress : '',
			vehicleBarter: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleBarter : false,
			vehicleRent: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleRent : false,
			vehicleRooms: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleRooms : 0,
			vehicleBeds: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleBeds : 0,
			vehicleSquare: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleSquare : 0,
			vehicleDesc: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleDesc : '',
			vehicleImages: getvehicleData?.getvehicle ? getvehicleData?.getvehicle?.vehicleImages : [],
		});
	}, [getvehicleLoading, getvehicleData]);
	...
	async function uploadImages() {
		try {
			...
			setInsertvehicleData({ ...insertvehicleData, vehicleImages: responseImages });
		} catch (err: any) {
	...
	const doDisabledCheck = () => {
		if (
			insertvehicleData.vehicleTitle === '' ||
			insertvehicleData.vehiclePrice === 0 || // @ts-ignore
			insertvehicleData.VehicleType === '' || // @ts-ignore
			insertvehicleData.VehicleLocation === '' || // @ts-ignore
			insertvehicleData.vehicleAddress === '' || // @ts-ignore
			insertvehicleData.vehicleBarter === '' || // @ts-ignore
			insertvehicleData.vehicleRent === '' ||
			insertvehicleData.vehicleRooms === 0 ||
			insertvehicleData.vehicleBeds === 0 ||
			insertvehicleData.vehicleSquare === 0 ||
			insertvehicleData.vehicleDesc === '' ||
	...
	const insertvehicleHandler = useCallback(async () => {}, [insertvehicleData]);

	const updatevehicleHandler = useCallback(async () => {}, [insertvehicleData]);

	if (user?.memberType !== 'dealer') {
		router.back();
	}

	console.log('+insertvehicleData', insertvehicleData);

	if (device === 'mobile') {
		return <div>ADD NEW vehicle MOBILE PAGE</div>;
	} else {
		return (
			<div id="add-vehicle-page">
					...
					<Stack className="config">
						<Stack className="description-box">
							<Stack className="config-column">
								...
								<input
									...
									placeholder={'Title'}
									value={insertvehicleData.vehicleTitle}
									onChange={({ target: { value } }) =>
										setInsertvehicleData({ ...insertvehicleData, vehicleTitle: value })
									}
								/>
							</Stack>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<input
										...
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehiclePrice: parseInt(value) })
										}
									/>
								</Stack>
								<Stack className="price-year-after-price">
									...
									<select
										...
										defaultValue={insertvehicleData.VehicleType || 'select'}
										value={insertvehicleData.VehicleType || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertvehicleData({ ...insertvehicleData, VehicleType: value })
										}
									>
										<>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										defaultValue={insertvehicleData.VehicleLocation || 'select'}
										value={insertvehicleData.VehicleLocation || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertvehicleData({ ...insertvehicleData, VehicleLocation: value })
										}
									>
										<>
											<option selected={true} disabled={true} value={'select'}>
											...
											{vehicleLocation.map((location: any) => (
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										className="description-input"
										placeholder={'Address'}
										value={insertvehicleData.vehicleAddress}
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehicleAddress: value })
										}
									/>
								</Stack>
							</Stack>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertvehicleData.vehicleBarter ? 'yes' : 'no'}
										defaultValue={insertvehicleData.vehicleBarter ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehicleBarter: value === 'yes' })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertvehicleData.vehicleRent ? 'yes' : 'no'}
										defaultValue={insertvehicleData.vehicleRent ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehicleRent: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertvehicleData.vehicleRooms || 'select'}
										defaultValue={insertvehicleData.vehicleRooms || 'select'}
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehicleRooms: parseInt(value) })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertvehicleData.vehicleBeds || 'select'}
										defaultValue={insertvehicleData.vehicleBeds || 'select'}
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehicleBeds: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertvehicleData.vehicleSquare || 'select'}
										defaultValue={insertvehicleData.vehicleSquare || 'select'}
										onChange={({ target: { value } }) =>
											setInsertvehicleData({ ...insertvehicleData, vehicleSquare: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
							...
							<Typography className="vehicle-title">vehicle Description</Typography>
							<Stack className="config-column">
								...
								<textarea
									...
									onChange={({ target: { value } }) =>
										setInsertvehicleData({ ...insertvehicleData, vehicleDesc: value })
									}
								></textarea>
							</Stack>
						</Stack>

						<Typography className="upload-title">Upload photos of your vehicle</Typography>
						...
						<Stack className="buttons-row">
							{router.query.vehicleId ? (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={updatevehicleHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							) : (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={insertvehicleHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							)}
						</Stack>
